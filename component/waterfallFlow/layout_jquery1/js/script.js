...
var waterfall = function(wrap, boxes) {
	...
	var boxWidth = boxes.eq(0).width() + 40; //width() 是内容的宽度
	//获取视口宽度
	...
	for (var i = 0; i < boxes.length; i++) {
		if (i < colsNumber) {
			everyHeight[i] = boxes.eq(i).height() + 40; //height()是内容的高度
		} else {
...
var getCheck = function(wrap) {
	...
	var lastHeight = boxes.eq(boxes.length - 1).height() + 20; //20是底部外边距+底部内边距
...
var appendBox = function(wrap) {
	// wrap.append('<div><img src="images/5.png"><a href="http://www.imooc.com" target="_blank">第五怪 火车没有汽车快<\/a><\/div>');
	if (getCheck(wrap)) {
		for (i in data) {
